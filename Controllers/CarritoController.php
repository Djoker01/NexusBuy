        ...
        if (!empty($_SESSION['id'])) {
            ...
            foreach ($carrito->objetos as $objeto) {
                $json[] = array(
                    ...
                    'descuento_final' => $objeto->descuento_unitario,
                    'cantidad' => $objeto->stock_disponible, // stock disponible
...
if ($_POST['funcion'] == 'agregar_al_carrito') {
    ...
    $id_usuario = $_SESSION['id'];
    $id_producto_tienda_encrypted = $_POST['id_producto_tienda']; // CORREGIDO: mismo nombre que en JS
    $cantidad = intval($_POST['cantidad'] ?? 1);
    
    error_log("Datos recibidos - Usuario: $id_usuario, ProductoEnc: $id_producto_tienda_encrypted, Cantidad: $cantidad");
    
    // USAR EL MISMO MÉTODO QUE EN ProductoTiendaController.php
    ...
    error_log("Es numérico: " . (is_numeric($id_producto_tienda) ? 'SI' : 'NO'));
...
if ($_POST['funcion'] == 'actualizar_cantidad') {
    try {
        if (!empty($_SESSION['id'])) {
            ...
            error_log("Actualizar cantidad - Usuario: $id_usuario, Item: $id_carrito_detalle, Cantidad: $nueva_cantidad");
