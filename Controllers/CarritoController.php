    ...
    try {
        ...
        foreach ($items as $objeto) {
            $json[] = array(
                ...
                'stock_disponible' => intval($objeto->stock_disponible), // CORREGIDO: nombre correcto
                ...
                'envio' => $objeto->envio ? 'Gratis' : 'Con costo'
            );
        }
...
if ($_POST['funcion'] == 'agregar_al_carrito') {
    ...
    // Desencriptar ID (usando el mismo método que en otros controladores)
    try {
...
if ($_POST['funcion'] == 'actualizar_cantidad') {
    try {
        ...
        $id_usuario = $_SESSION['id'];
        $id_carrito = $_POST['id_carrito_detalle'] ?? null; // CORREGIDO: nombre más apropiado
        $nueva_cantidad = isset($_POST['cantidad']) ? intval($_POST['cantidad']) : null;
        
        error_log("Actualizar cantidad - Usuario: $id_usuario, Item: $id_carrito, Cantidad: $nueva_cantidad");
