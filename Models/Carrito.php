...
class Carrito
{
    ...
    function obtener_carrito($id_usuario) {
        $sql = "SELECT 
                    ci.id,
                    ci.cantidad as cantidad_producto,  
                    ci.precio_unitario,
                    ...
                    pt.descuento as descuento_porcentaje,  
                    p.id as id_producto  
                FROM carrrito c
    ...
    function actualizar_cantidad($id_carrito_detalle, $nueva_cantidad) {
        $this->acceso->beginTransaction();
        try {
            // 1. Verificar que el item existe y obtener información
            $sql = "SELECT ci.id, ci.precio_unitario, ci.descuento_unitario, 
                           pt.cantidad as stock, pt.id as id_producto_tienda
                    FROM carrito_items ci
                    JOIN producto_tienda pt ON ci.id_producto_tienda = pt.id
                    WHERE ci.id = :id_item";
            
            $query = $this->acceso->prepare($sql);
            $query->execute(array(':id_item' => $id_carrito_detalle));
            $item = $query->fetch();
            ...
            $this->acceso->rollBack();
            error_log("Error en actualizar_cantidad modelo: " . $e->getMessage());
            throw $e; // Relanzar la excepción para que el controller la capture
        }
    }
