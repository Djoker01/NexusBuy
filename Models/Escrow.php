...
class Escrow {
    ...
    try {
        ...
        // Verificar saldo disponible - AQUÍ TAMBIÉN PUEDE HABER ERROR
    ...
    /**
 * Obtener saldo disponible de un vendedor por ID de usuario
 * @param int $id_usuario ID del usuario/vendedor
 * @return float Saldo disponible
 */
public function getSaldoDisponible($id_usuario) {
    try {
        // Primero obtenemos la tienda del usuario
        $sql_tienda = "SELECT id FROM tienda WHERE id_usuario = :id_usuario LIMIT 1";
        $query_t = $this->acceso->prepare($sql_tienda);
        $query_t->execute([':id_usuario' => $id_usuario]);
        $tienda = $query_t->fetch(PDO::FETCH_OBJ);
        
        if (!$tienda) {
            // El usuario no tiene tienda, saldo 0
            return 0.00;
        }
        
        // Consultar saldo disponible
        $sql = "SELECT saldo_disponible 
                FROM saldo_vendedor 
                WHERE id_tienda = :id_tienda";
        
        $query = $this->acceso->prepare($sql);
        $query->execute([':id_tienda' => $tienda->id]);
        $resultado = $query->fetch(PDO::FETCH_OBJ);
        
        if ($resultado) {
            return floatval($resultado->saldo_disponible);
        } else {
            // Si no existe registro de saldo, crearlo
            return $this->crearSaldoInicial($tienda->id, $id_usuario);
        }
        
    } catch (PDOException $e) {
        $this->error = $e->getMessage();
        error_log("Error en getSaldoDisponible: " . $e->getMessage());
        return 0.00;
    }
}

/**
 * Crear registro de saldo inicial para un vendedor
 */
private function crearSaldoInicial($id_vendedor) {
    try {
        error_log("=== CREAR SALDO INICIAL ===");
        error_log("Creando saldo para id_vendedor: " . $id_vendedor);
        
        // Buscar la tienda del vendedor - CORREGIDO
        $sql_tienda = "SELECT id FROM tienda WHERE id_usuario_propietario = :id_usuario";
        $query_t = $this->acceso->prepare($sql_tienda);
        $query_t->execute([':id_usuario' => $id_vendedor]);
        $tienda = $query_t->fetch(PDO::FETCH_OBJ);
        
        if (!$tienda) {
            error_log("ERROR: El vendedor no tiene tienda creada");
            return 0.00;
        }
        
        error_log("Tienda encontrada: id = " . $tienda->id);
        
        // Verificar si ya existe un registro
        $sql_check = "SELECT id FROM saldo_vendedor WHERE id_vendedor = :id_vendedor";
        $query_c = $this->acceso->prepare($sql_check);
        $query_c->execute([':id_vendedor' => $id_vendedor]);
        
        if ($query_c->fetch()) {
            error_log("Ya existe un registro de saldo para este vendedor");
            return 0.00;
        }
        
        // Crear nuevo registro
        $sql = "INSERT INTO saldo_vendedor 
                (id_tienda, id_vendedor, saldo_actual, saldo_retenido, saldo_disponible, 
                 total_ganado, total_retirado, fecha_actualizacion) 
                VALUES 
                (:id_tienda, :id_vendedor, 0, 0, 0, 0, 0, NOW())";
        ...
        if ($resultado) {
            error_log("✅ Registro de saldo creado exitosamente");
        } else {
            error_log("❌ Error al crear registro de saldo");
        }
        ...
        $this->error = $e->getMessage();
        error_log("Error en crearSaldoInicial: " . $e->getMessage());
        return 0.00;
    }
}

/**
 * Versión simplificada de getSaldoDisponible que busca por id_vendedor directamente
 * (Útil si tu tabla saldo_vendedor ya tiene el id_vendedor)
 */
public function getSaldoPorVendedor($id_vendedor) {
    try {
        error_log("=== DEBUG getSaldoPorVendedor ===");
        error_log("Buscando saldo para id_vendedor: " . $id_vendedor);
        
        // Buscar directamente por id_vendedor (ya existe)
        $sql = "SELECT saldo_disponible 
                FROM saldo_vendedor 
                WHERE id_vendedor = :id_vendedor";
        
        $query = $this->acceso->prepare($sql);
        $query->execute([':id_vendedor' => $id_vendedor]);
        $resultado = $query->fetch(PDO::FETCH_OBJ);
        
        if ($resultado) {
            error_log("✅ Saldo encontrado directamente: $" . $resultado->saldo_disponible);
            return floatval($resultado->saldo_disponible);
        }
        
        // Si no encuentra, buscar la tienda del usuario
        error_log("Buscando tienda para usuario: " . $id_vendedor);
        $sql_tienda = "SELECT id FROM tienda WHERE id_usuario_propietario = :id_usuario"; // ← CORREGIDO
        $query_t = $this->acceso->prepare($sql_tienda);
        $query_t->execute([':id_usuario' => $id_vendedor]);
        $tienda = $query_t->fetch(PDO::FETCH_OBJ);
        ...
        if ($tienda) {
            ...
            $sql_saldo = "SELECT saldo_disponible FROM saldo_vendedor WHERE id_tienda = :id_tienda";
            $query_s = $this->acceso->prepare($sql_saldo);
            $query_s->execute([':id_tienda' => $tienda->id]);
            $resultado_tienda = $query_s->fetch(PDO::FETCH_OBJ);
            
            if ($resultado_tienda) {
                error_log("✅ Saldo encontrado por tienda: $" . $resultado_tienda->saldo_disponible);
                return floatval($resultado_tienda->saldo_disponible);
            }
        }
        
        error_log("❌ No se encontró saldo para el usuario " . $id_vendedor);
        return 0.00;
        
    } catch (PDOException $e) {
        $this->error = $e->getMessage();
        error_log("Error en getSaldoPorVendedor: " . $e->getMessage());
        return 0.00;
    }
}

/**
 * Método específico para descontar saldo por banners
 * (Adaptado de tu descontarServicio existente)
 */
public function descontarBanner($id_usuario, $monto, $concepto, $id_banner) {
    try {
        $this->acceso->beginTransaction();
        
        // Obtener id_tienda del usuario
        $sql_tienda = "SELECT id FROM tienda WHERE id_usuario = :id_usuario";
        $query_t = $this->acceso->prepare($sql_tienda);
        $query_t->execute([':id_usuario' => $id_usuario]);
        $tienda = $query_t->fetch(PDO::FETCH_OBJ);
        ...
        // Usar tu método existente descontarServicio
        $resultado = $this->descontarServicio($tienda->id, $monto, $concepto, $id_banner);
        ...
        $this->acceso->rollBack();
        $this->error = $e->getMessage();
        error_log("Error en descontarBanner: " . $e->getMessage());
        return false;
    }
}

/**
 * Registrar movimiento (si no existe)
 */
private function registrarMovimiento($id_tienda, $tipo, $concepto, $monto, $referencia_id, $referencia_tipo) {
    try {
        // Verificar si existe la tabla movimientos
        $sql_check = "SHOW TABLES LIKE 'movimientos_saldo'";
        $query = $this->acceso->query($sql_check);
        
        if ($query->rowCount() > 0) {
            $sql = "INSERT INTO movimientos_saldo 
                    (id_tienda, tipo, concepto, monto, referencia_id, referencia_tipo, fecha) 
                    VALUES 
                    (:id_tienda, :tipo, :concepto, :monto, :referencia_id, :referencia_tipo, NOW())";
            
            $query_i = $this->acceso->prepare($sql);
            $query_i->execute([
                ':id_tienda' => $id_tienda,
                ':tipo' => $tipo,
                ':concepto' => $concepto,
