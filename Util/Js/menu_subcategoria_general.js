...
window.filtrarPorCategoria = async function(id_categoria, nombre_categoria) {
  try {
      console.log('Filtrando por categoría:', id_categoria, nombre_categoria);
      ...
      $('#productos').html(`
          <div class="col-12 text-center py-5">
              ...
              <p class="text-muted mt-2">Cargando productos de ${nombre_categoria}...</p>
          </div>
      `);

      $('.card-title').html(`Productos - ${nombre_categoria} <span class="badge bg-secondary">Cargando...</span>`);
      document.title = `${nombre_categoria} | NexusBuy`;

      $('.breadcrumb').html(`
          <li class="breadcrumb-item"><a href="index.php">Home</a></li>
          <li class="breadcrumb-item active">${nombre_categoria}</li>
      `);
      ...
      const response = await fetch('../Controllers/ProductoTiendaController.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: `funcion=${funcion}&id_categoria=${id_categoria}`
      });

      if (response.ok) {
          const data = await response.text();
          const productos = JSON.parse(data);
          mostrarProductosFiltrados(productos, nombre_categoria);
      } else {
  ...
  categorias.forEach(categoria => {
      ...
      menuHTML += `
          <li class="${menuItemClass}">
              <a href="../index.php" class="${menuLinkClass}" ${!hasSubcategories ? `onclick="filtrarPorCategoria(${categoria.id}, '${categoria.nombre.replace(/'/g, "\\'")}')"` : ''}>
      ...
      if (hasSubcategories) {
          ...
          categoria.subcategorias.forEach(subcategoria => {
              menuHTML += `
                  <li class="nav-item">
                      <a href="../index.php" class="nav-link" onclick="filtrarPorSubcategoria(${subcategoria.id}, '${subcategoria.nombre.replace(/'/g, "\\'")}')">
