$(document).ready(function () {
    ...
    function renderizarCarrito() {
        ...
        carritoItems.forEach((item, index) => {
            ...
            const envioItem = item.envio_gratis == 1 ? 0 : parseFloat(item.costo_envio) || 0;
        ...
        try {
            ...
            if (data.success) {
                ...
                if (item) {
                    ...
                    const itemElement = $(`.articulo-item[data-carrito-id="${carritoId}"]`);
    ...
    function actualizarEstadoBotones() {
        ...
        carritoItems.forEach((item) => {
            ...
            const btnDisminuir = $(`.disminuir-cantidad[data-carrito-id="${item.id}"]`);
    ...
    async function actualizarResumen() {
        ...
        seleccionados.forEach(carritoId => {
            ...
            if (item) {
                ...
                const precioUnitario = item.precio_unitario_convertido || parseFloat(item.precio_unitario) || parseFloat(item.precio_final) || 0;
                const precioFinal = item.precio_final_convertido || parseFloat(item.precio_final) || 0;
                const cantidad = parseInt(item.cantidad_producto) || 1;
                const envioItem = parseFloat(item.envio) || 0;
        ...
        const umbralEnvioGratis = 10000000 / tasaCambioActual; // Convertir umbral a moneda actual
        let envioFinal = 0;
        ...
        $('#subtotal').text(`${subtotalSinDescuento.toFixed(2)} ${simboloMonedaActual}`);

        if (descuentoTotal > 0) {
            $('#descuento').text(`-${descuentoTotal.toFixed(2)} ${simboloMonedaActual} `);
    ...
    function procederAlPago() {
        ...
        let subtotalSinDescuentoCheckout = 0;
        let subtotalConDescuentoCheckout = 0;
        let envioCheckout = 0;
        let descuentoCheckout = 0;
        
        seleccionados.forEach(carritoId => {
            const item = carritoItems.find(item => item.id.toString() === carritoId.toString());
            if (item) {
                const precioUnitario = item.precio_unitario_convertido || parseFloat(item.precio_unitario) || parseFloat(item.precio_final) || 0;
                const precioFinal = item.precio_final_convertido || parseFloat(item.precio_final) || 0;
                const cantidad = parseInt(item.cantidad_producto) || 1;
                ...
                subtotalSinDescuentoCheckout += subtotalItemSinDescuento;
                subtotalConDescuentoCheckout += subtotalItemConDescuento;
        ...
        const umbralEnvioGratis = 10000000 / tasaCambioActual;
        if (subtotalConDescuentoCheckout > umbralEnvioGratis) {
            envioCheckout = 0;
        }
        ...
        Swal.fire({
            ...
            html: `
                <div class="text-left">
                    ...
                    <div class="mt-3 p-2 bg-light rounded">
                        <div class="row">
                            <div class="col-6"><strong>Subtotal original:</strong></div>
                            <div class="col-6 text-right">${subtotalSinDescuentoCheckout.toFixed(2)} ${simboloMonedaActual}</div>
                        </div>
                        ${descuentoCheckout > 0 ? `
                        <div class="row">
            ...
            if (result.isConfirmed) {
                ...
                sessionStorage.setItem('checkoutSubtotalSinDescuento', subtotalSinDescuentoCheckout.toString());
                sessionStorage.setItem('checkoutSubtotalConDescuento', subtotalConDescuentoCheckout.toString());
        ...
        try {
            ...
            if (data.success) {
                ...
                carritoItems.forEach(item => {
                    ...
                    item.descuento_convertido = (item.precio_unitario_convertido - item.precio_final_convertido) * cantidad;
                ...
                $('.articulo-item').each(function() {
                    ...
                    if (item && item.precio_final_convertido) {
                        ...
                        $precioProducto.find('.current-price').text(
