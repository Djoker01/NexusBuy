$(document).ready(function () {
  ...
  function renderizarFavoritosGrid() {
    ...
    favoritos.forEach((producto) => {
      const precioFinal =
        parseFloat(producto.precio_descuento) || parseFloat(producto.precio);
                    ...
                    <div class="card favorite-card">
                        <div class="product-image">
                            ...
                            <button class="favorite-badge" 
                                    onclick="toggleFavoritoDesdeCard('${
                                      producto.id_encrypted
                                    }')"
                        ...
                        <div class="product-info">
                            ...
                            <div class="pricing">
                                ${
                                  tieneDescuento
                                    ? `
                                    <span class="original-price">$${precioOriginal.toFixed(
                                      2
                                    )}</span>
                                    <span class="current-price">$${precioFinal.toFixed(
                                      2
                                    )}</span>
                                `
                                    : `
                            ...
                            <div class="product-actions">
                                ...
                                <button class="btn-quick-view" onclick="window.location.href='descripcion.php?name=${encodeURIComponent(producto.producto)}&id=${producto.id}'">>
  ...
  function renderizarFavoritosLista() {
    ...
    favoritos.forEach((producto) => {
      const precioFinal =
        parseFloat(producto.precio_descuento) || parseFloat(producto.precio);
      const fecha = new Date(producto.fecha_agregado).toLocaleDateString(
        "es-ES"
      );
      const tieneDescuento = producto.descuento > 0;
  ...
  async function cargarCategorias() {
    //console.log("Cargando categorías de favoritos...");
    try {
      const response = await $.post("../Controllers/FavoritoController.php", {
      ...
      try {
        ...
        console.error("Error parseando JSON de categorías:", parseError);
        return;
      }
      ...
      if (data.success && data.categorias) {
        ...
        //console.log("Categorias cargadas:", data.categorias.length);
      } else {
        console.error("Error cargando categorías:", data.error);
      }
    } catch (error) {
      console.error("Error en cargarCategorías:", error);
    }
  }
  ...
  function exportarFavoritos() {
    ...
    favoritos.forEach((producto) => {
      const precioFinal =
        parseFloat(producto.precio_descuento) || parseFloat(producto.precio);
      csvContent += `"${producto.producto}","${producto.marca}","${
        producto.categoria
      }",${precioFinal.toFixed(2)},${producto.calificacion}\n`;
    });
...
async function toggleFavorito(idProducto) {
  ...
  try {
    const responseVerificar = await $.post(
    ...
    const dataVerificar =
      typeof responseVerificar === "string"
        ? JSON.parse(responseVerificar)
        : responseVerificar;
...
function compartirFacebook() {
  ...
  const texto = encodeURIComponent(
    "¡Mira mis productos favoritos en NexusBuy!"
  );
  window.open(
    `https://www.facebook.com/sharer/sharer.php?u=${enlace}&quote=${texto}`,
    "_blank"
  );
}
...
function compartirTwitter() {
  ...
  window.open(
    `https://twitter.com/intent/tweet?text=${texto}&url=${enlace}`,
    "_blank"
  );
}
...
function compartirPinterest() {
  ...
  window.open(
    `https://pinterest.com/pin/create/button/?url=${enlace}&description=${descripcion}`,
    "_blank"
  );
}
