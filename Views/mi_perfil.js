...
function aplicarConfiguracion(configuraciones) {
    ...
    if (configuraciones.notificaciones) {
        $("#notificacion-email").prop("checked", configuraciones.notificaciones.email || false);
...
// ========== FUNCIÓN GUARDAR CONFIGURACIÓN CORREGIDA ==========
async function guardarConfiguracion(tipo, datos) {
    ...
    try {
        ...
        if (response.success) {
            // Éxito
            mostrarExito(response.mensaje || "Configuración guardada correctamente");
            ...
            if (tipo === "visualizacion") {
                ...
                // Actualizar moneda si cambió
                if (datos.moneda) {
                    localStorage.setItem("moneda-seleccionada", datos.moneda);
                    actualizarPreciosMoneda(datos.moneda);
                }
            }
        ...
        restaurarValoresConfiguracion(tipo);
    } finally {
        ocultarLoadingGuardado(tipo);
    }
}

// Función auxiliar para mostrar loading
function mostrarLoadingGuardado(tipo) {
    let btnSelector;
    switch(tipo) {
        case 'notificaciones':
            btnSelector = '#form-notificaciones button[type="submit"]';
            break;
        case 'privacidad':
            btnSelector = '#form-privacidad button[type="submit"]';
            break;
        case 'visualizacion':
            btnSelector = '#form-visualizacion button[type="submit"]';
            break;
        default:
            return;
    }
...
// Función auxiliar para ocultar loading
function ocultarLoadingGuardado(tipo) {
    let btnSelector;
    switch(tipo) {
        case 'notificaciones':
            btnSelector = '#form-notificaciones button[type="submit"]';
...
// Función para restaurar valores en caso de error
function restaurarValoresConfiguracion(tipo) {
