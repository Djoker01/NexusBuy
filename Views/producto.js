document.addEventListener("DOMContentLoaded", function () {
  ...
  // Cargar productos (MODIFICADA para aceptar marca)
  async function loadProducts() {
    try {
       ...
       if (filtroParam === 'nuevos') {
            ...
            console.log("Filtrando por productos nuevos");
        } else if (marcaParam) {
            bodyParams.append('nombre_marca', marcaParam);
            console.log("Filtrando por marca:", marcaParam);
        } else if (subcategoriaParam) {
            // Primero buscar el ID de la subcategoría por nombre
  ...
  // Función para mostrar mensaje cuando no hay productos (MODIFICADA para marca)
  function showNoProductsMessage(categoriaNombre = null, subcategoriaNombre = null) {
    ...
    if (marcaParam) {
        ...
        message = `No se encontraron productos de la marca "${decodeURIComponent(marcaParam)}".`;
    } else if (categoriaNombre) {
        title = 'Categoría vacía';
        message = `No se encontraron productos en la categoría "${decodeURIComponent(categoriaNombre)}".`;
    } else if (subcategoriaNombre) {
        title = 'Subcategoría vacía';
        message = `No se encontraron productos en la subcategoría "${decodeURIComponent(subcategoriaNombre)}".`;
    }
  ...
  // Función para mostrar productos (SE MANTIENE INTACTA)
  ...
  // Función para actualizar contador de resultados (MODIFICADA para marca)
  function updateResultsCount(count, categoriaNombre = null, subcategoriaNombre = null) {
    ...
    if (resultsElement) {
       ...
       if (filtroParam === 'nuevos') {
            ...
            message = `${count} productos en "${decodeURIComponent(categoriaNombre)}"`;
        } else if (subcategoriaNombre) {
            message = `${count} productos en "${decodeURIComponent(subcategoriaNombre)}"`;
        } else {
...
// Función para actualizar título de página según filtro (MODIFICADA para marca)
...
// Resto de las funciones globales se mantienen EXACTAMENTE igual:
