$(document).ready(function () {
  ...
  $(document).on(
    "click",
    ".pagination-marcas .page-link[data-pagina]",
    function (e) {
      e.preventDefault();
  ...
  async function marcas(pagina = 1, limite = 50) {
    ...
    try {
      ...
      try {
        ...
        if (
          resultado.marcas &&
          Array.isArray(resultado.marcas) &&
          resultado.marcas.length > 0
        ) {
          resultado.marcas.forEach((marca) => {
            templateMarcas += `
                        <div class="brand-item">
                            <a href="Views/producto.php?marca=${encodeURIComponent(
                              marca.nombre
                            )}" class="brand-link">
                                <img src="Util/Img/Marca/${marca.logo}" alt="${
              marca.nombre
            }" class="img-fluid">
  ...
  function generarPaginacionMarcas(paginaActual, totalPaginas, totalMarcas) {
    ...
    if ($("#paginacion-marcas").length) {
      ...
      $("#marca_destacada")
        .closest(".card")
        .after(`<div id="paginacion-marcas">${paginacionHTML}</div>`);
    }
  }
